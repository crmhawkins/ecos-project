                ...
                <div class="card-body">
                    <form action="{{route('passwords.update',$password->id)}}" method="POST">
                                  ...
                                  @if ($clientes->count() > 0)
                                      <option value="{{null}}">--- Seleccione un cliente ---</option>
                                        @foreach ( $clientes as $cliente )
                                            <option {{old('client_id',$password->client_id) == $cliente->id ? 'selected' : '' }} data-id="{{$cliente->id}}" value="{{$cliente->id}}">{{$cliente->name}}</option>
                                        @endforeach
                                    @else
                        ...
                        <div class="form-group mb-4">
                            <label class="text-uppercase" style="font-weight: bold" for="website">Web:</label>
                            <input type="text" class="form-control @error('website') is-invalid @enderror" id="website" value="{{ old('website',$password->website) }}" name="website">
                            @error('website')
                        ...
                        <div class="form-group mb-4">
                            <label class="text-uppercase" style="font-weight: bold" for="user">Usuario:</label>
                            <input type="text" class="form-control @error('user') is-invalid @enderror" id="user" value="{{ old('user',$password->user) }}" name="user">
                            @error('user')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        ...
                        <div class="form-group mb-4">
                            <label class="text-uppercase" style="font-weight: bold" for="password">Contraseña:</label>
                            <input type="text" class="form-control @error('password') is-invalid @enderror" id="password" value="{{ old('password',$password->password) }}" name="password">
                            @error('password')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
