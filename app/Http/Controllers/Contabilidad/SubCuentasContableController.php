...
class SubCuentasContableController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($search) {
            $query->where('nombre', 'like', '%' . $search . '%')
                  ...
                  ->orWhereHas('cuenta_id', function ($q) use ($search) {
                      $q->where('nombre', 'like', '%' . $search . '%');
                  });
        }

        if ($subGrupo) {
            $query->where('cuenta_id', $subGrupo);
        }

        $response = $query->orderBy($sort, $order)->paginate($perPage);

        $subCuentas = CuentasContable::all();
        return view('crm.contabilidad.subCuentaContabilidad.index', compact('response', 'subCuentas'));
    }
    ...
    public function create()
    {
        ...
        return view('crm.contabilidad.cuentaContabilidad.create', compact('subgrupos', 'grupos'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('crm.contabilidad.cuentaContabilidad.edit', compact('cuenta', 'grupo'));
