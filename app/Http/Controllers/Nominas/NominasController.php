...
class NominasController extends Controller
{
    public function index()
    {
        ...
        if(Auth::user()->access_level_id == 1 || Auth::user()->access_level_id == 2 || Auth::user()->access_level_id == 3  ){
            ...
            return view('crm.nominas.index', compact('nominas'));
        }else{
    ...
    {
        if ($id == Auth::user()->id){
            return view('crm.nominas.index_user', compact('id'));
        }else{
            return redirect()->back()->with('toast', [
                'icon' => 'error',
                'mensaje' => 'No tienes permiso para acceder']);
    ...
    public function show($id)
    {
        ...
        if( $nomina->admin_user_id == Auth::user()->id){
            return view('crm.nominas.show', compact('nomina'));
        }else{
            return redirect()->back()->with('toast', [
                'icon' => 'error',
                'mensaje' => 'No tienes permiso para acceder']);
    ...
    public function edit($id)
    {
        ...
        if( $nomina->admin_user_id == Auth::user()->id || Auth::user()->access_level_id == 1 || Auth::user()->access_level_id == 2 || Auth::user()->access_level_id == 3  ){
            return view('crm.nominas.edit', compact('nomina','usuarios'));
        }else{
            return redirect()->back()->with('toast', [
                'icon' => 'error',
                'mensaje' => 'No tienes permiso para acceder']);
    ...
    {
        $usuarios = User::all();
        return view('crm.nominas.create', compact('usuarios'));
    }
