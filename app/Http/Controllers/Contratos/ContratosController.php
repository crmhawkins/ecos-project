...
class ContratosController extends Controller
{
    public function index()
    {
        if(Auth::user()->access_level_id == 1 || Auth::user()->access_level_id == 2 || Auth::user()->access_level_id == 3  ){
            ...
            return view('crm.contratos.index', compact('contratos'));
        }else{
    ...
    {
        if ($id == Auth::user()->id){
            return view('crm.contratos.index_user', compact('id'));
        }else{
            return redirect()->back()->with('toast', [
                'icon' => 'error',
                'mensaje' => 'No tienes permiso para acceder']);
    ...
    public function show($id)
    {
        ...
        if( $contrato->admin_user_id == Auth::user()->id || Auth::user()->access_level_id == 1 || Auth::user()->access_level_id == 2 || Auth::user()->access_level_id == 3 ){
            return view('crm.contratos.show', compact('contrato'));
        }else{
            return redirect()->back()->with('toast', [
                'icon' => 'error',
                'mensaje' => 'No tienes permiso para acceder']);
    ...
    public function edit($id)
    {
        ...
        $usuarios = User::all();
        if( $contrato->admin_user_id == Auth::user()->id || Auth::user()->access_level_id == 1 || Auth::user()->access_level_id == 2 || Auth::user()->access_level_id == 3  ){
            return view('crm.contratos.edit', compact('contrato','usuarios'));
        }else{
            return redirect()->back()->with('toast', [
                'icon' => 'error',
                'mensaje' => 'No tienes permiso para acceder']);
    ...
    public function create()
    {
        ...
        $usuarios = User::all();
        return view('crm.contratos.create', compact('usuarios'));
    }
